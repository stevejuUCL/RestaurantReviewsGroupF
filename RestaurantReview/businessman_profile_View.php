...
<section class="panel">
    ...
    <div style="text-align: left; width: 50%; margin: auto;">
        ...
        $query = "SELECT name, user_name, email, company_name, contact_number,  img_url
              FROM userinfo 
              ";
...
<!----------------------Average rating---------------------------->
        <?php
        $conn = new mysqli("localhost", "steveju", "stevejuju546", "redwine");
        $query = "SELECT businessman_review.rating FROM userinfo INNER JOIN businessman_review
                    ON userinfo.user_name = businessman_review.name ";
        ...
        <h4>Average Rating: <?php echo ($total/$num);?></h4>
<!---------------------------------------------------------------->
<!----------------------review---------------------------->
    <?php
          $conn = new mysqli("localhost", "steveju", "stevejuju546", "redwine");
          $query = "SELECT businessman_review.review FROM userinfo INNER JOIN businessman_review
                    ON userinfo.user_name = businessman_review.name
